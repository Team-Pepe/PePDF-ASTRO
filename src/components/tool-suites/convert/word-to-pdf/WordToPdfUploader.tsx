...
export default function WordToPdfUploader() {
  ...
  const onFileChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    const [first] = selected.filter((item) => ACCEPTED_TYPES.includes(item.type) || item.name.toLowerCase().endsWith('.docx') || item.name.toLowerCase().endsWith('.doc'));

    if (!first) {
      setError('Please select a supported Word document (.doc or .docx).');
      return;
    }
  ...
  const handleConvert = async () => {
    ...
    try {
      const pdfBlob = await convertToolsService.wordToPdf(file);
      if (downloadUrl) {
        URL.revokeObjectURL(downloadUrl);
      }
    ...
    <section className="pb-10 px-6">
      <div className="max-w-6xl mx-auto glass-card bg-white/60 dark:bg-white/5 rounded-3xl p-8 md:p-10">
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-6 mb-8">
          <div>
            ...
            <p className="text-slate-600 dark:text-slate-300 max-w-2xl">
              Upload a Word document and convert it to PDF using LibreOffice running inside Docker.
            </p>
          </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          {downloadUrl && (
            <a href={downloadUrl} download="converted.pdf" className="btn-primary">
