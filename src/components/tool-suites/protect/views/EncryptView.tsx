...
import { encryptPdf } from '../../../../services/protect';
...
export default function EncryptView() {
  ...
  const handleEncrypt = async () => {
    ...
    try {
      ...
      const blob = await encryptPdf(form);
  ...
  return (
    <>
      <ProtectLocalNav current="encrypt" />
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-5 gap-8">
          <div className="lg:col-span-3 glass-card bg-white/70 dark:bg-white/5 rounded-3xl p-7 border border-slate-200/80 dark:border-white/10">
            ...
            <p className="text-slate-600 dark:text-slate-300 mb-7">
              Add a strong opening password to your PDF. Keep owner password safe to change permissions later.
            </p>
            ...
            <div className="space-y-5">
              ...
              <div>
                <label className="auth-label">Owner Password (required to change later)</label>
                <input value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Set owner password" className="auth-input" />
              </div>

              <div className="flex gap-3 pt-2">
                <button type="button" className="btn-primary" onClick={handleEncrypt} disabled={!file || loading}>
            ...
            <div className="rounded-2xl border border-dashed border-slate-300 dark:border-white/15 min-h-[200px] flex flex-col items-center justify-center p-4 bg-white/70 dark:bg-black/20">
              {resultUrl ? (
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400 text-center max-w-[16rem]">
                  Upload a PDF and set an owner password to encrypt it.
                </p>
              )}
            </div>
          </aside>
